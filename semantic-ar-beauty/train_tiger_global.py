...
from modeling.dataset.global_split import create_dataset_from_sasrec_pkl
...
def main():
    ...
    dataset = create_dataset_from_sasrec_pkl(
        pkl_path=config['dataset']['split_pkl_path'],
        max_sequence_length=config['dataset']['max_sequence_length'],
        sampler_type=config['dataset']['sampler_type'],
        is_extended=config['dataset'].get('is_extended', True),
        mode=config['dataset'].get('mode', 'tune')
    )
    ...
    if resume_path:
        LOGGER.debug(f'Loading checkpoint from {resume_path}')
        state_dict = torch.load(resume_path, map_location=utils.DEVICE)
    ...
    ranking_metrics = {
        ...
        'ndcg@100': NDCGSemanticMetric(100, codebook_size, num_codebooks),
        'recall@5': RecallSemanticMetric(5, codebook_size, num_codebooks),
        'recall@10': RecallSemanticMetric(10, codebook_size, num_codebooks),
        'recall@100': RecallSemanticMetric(100, codebook_size, num_codebooks)
    }

    LOGGER.debug('Everything is ready for TIGER global-split training!')
    ...
    trainer = Trainer(
        ...
        best_metric=config.get('best_metric', 'ndcg@10'),
        ...
        eval_during_train=config.get('eval_during_train', False)
    )
